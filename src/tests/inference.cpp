...
class InferenceTest : public ::testing::Test {
 protected:
  void SetUp() override {
    // Create a constituent table for testing
    constituent_table_ = make_constituent_table();
    ...
    tide_table_[Constituent::kQ1].first = Complex(0.1, 0.05);     // Diurnal
    tide_table_[Constituent::kO1].first = Complex(0.5, 0.2);      // Diurnal
    tide_table_[Constituent::kK1].first = Complex(0.8, 0.3);      // Diurnal
    tide_table_[Constituent::kN2].first = Complex(0.3, 0.1);      // Semidiurnal
    tide_table_[Constituent::kM2].first = Complex(1.0, 0.4);      // Semidiurnal
    tide_table_[Constituent::kS2].first = Complex(0.6, 0.2);      // Semidiurnal
    tide_table_[Constituent::kNode].first = Complex(0.02, 0.01);  // Long period
    tide_table_[Constituent::kSa].first = Complex(0.05, 0.02);    // Long period
    tide_table_[Constituent::kMm].first = Complex(0.08, 0.03);    // Long period
  }

  ConstituentTable constituent_table_;
...
TEST_F(InferenceTest, ConstructorLinearHarmonicMinor) {
  EXPECT_NO_THROW({
    Inference inference(InterpolationType::kLinearAdmittance,
                        InputType::kHormonic, CalculationType::kMinor,
                        constituent_table_);
  });
}
...
TEST_F(InferenceTest, ConstructorLinearAmplitudeMinor) {
  EXPECT_NO_THROW({
    Inference inference(InterpolationType::kLinearAdmittance,
                        InputType::kAmplitude, CalculationType::kMinor,
                        constituent_table_);
  });
}
...
TEST_F(InferenceTest, ConstructorLinearHarmonicLongPeriod) {
  EXPECT_NO_THROW({
    Inference inference(InterpolationType::kLinearAdmittance,
                        InputType::kHormonic, CalculationType::kMinorLongPeriod,
                        constituent_table_);
  });
}
...
TEST_F(InferenceTest, OperatorHarmonicMinor) {
  ...
  EXPECT_NE(tide_table_[Constituent::kRho1].first, Complex(0.0, 0.0));
  EXPECT_NE(tide_table_[Constituent::kTau1].first, Complex(0.0, 0.0));
  EXPECT_NE(tide_table_[Constituent::kNu2].first, Complex(0.0, 0.0));
  EXPECT_NE(tide_table_[Constituent::kMu2].first, Complex(0.0, 0.0));

  // Check that magnitudes are reasonable (all constituents should have some
  // magnitude)
  EXPECT_GT(std::abs(tide_table_[Constituent::kQ1].first), 0.0);
  EXPECT_GT(std::abs(tide_table_[Constituent::kO1].first), 0.0);
  EXPECT_GT(std::abs(tide_table_[Constituent::kK1].first), 0.0);
}
...
TEST_F(InferenceTest, OperatorAmplitudeMinor) {
  ...
  tide_table_[Constituent::kQ1].first =
      Complex(0.1, 30.0);  // amp=0.1, phase=30°
  tide_table_[Constituent::kO1].first =
      Complex(0.5, 45.0);  // amp=0.5, phase=45°
  tide_table_[Constituent::kK1].first =
      Complex(0.8, 60.0);  // amp=0.8, phase=60°
  tide_table_[Constituent::kN2].first =
      Complex(0.3, 15.0);  // amp=0.3, phase=15°
  tide_table_[Constituent::kM2].first =
      Complex(1.0, 90.0);  // amp=1.0, phase=90°
  tide_table_[Constituent::kS2].first =
  ...
  EXPECT_GT(std::abs(tide_table_[Constituent::kRho1].first), 0.0);
  EXPECT_GT(std::abs(tide_table_[Constituent::kTau1].first), 0.0);
}
...
TEST_F(InferenceTest, OperatorLongPeriod) {
  ...
  EXPECT_NE(tide_table_[Constituent::kSsa].first, Complex(0.0, 0.0));
  EXPECT_NE(tide_table_[Constituent::kMf].first, Complex(0.0, 0.0));
  EXPECT_NE(tide_table_[Constituent::kMSf].first, Complex(0.0, 0.0));
}
...
TEST_F(InferenceTest, ReasonableMagnitudes) {
  ...
  double m2_magnitude = std::abs(tide_table_[Constituent::kM2].first);
  double nu2_magnitude = std::abs(tide_table_[Constituent::kNu2].first);
  double mu2_magnitude = std::abs(tide_table_[Constituent::kMu2].first);
  ...
  double o1_magnitude = std::abs(tide_table_[Constituent::kO1].first);
  double rho1_magnitude = std::abs(tide_table_[Constituent::kRho1].first);
  double tau1_magnitude = std::abs(tide_table_[Constituent::kTau1].first);
...
TEST_F(InferenceTest, ZeroInputValues) {
  ...
  for (const auto& constituent : {Constituent::kRho1, Constituent::kTau1,
    ...
    EXPECT_DOUBLE_EQ(zero_table[constituent].first.real(), 0.0);
    EXPECT_DOUBLE_EQ(zero_table[constituent].first.imag(), 0.0);
  }
}
...
TEST_F(InferenceTest, InterpolationConsistency) {
  ...
  for (const auto& constituent : {Constituent::kRho1, Constituent::kTau1}) {
    double linear_mag = std::abs(linear_table[constituent].first);
    double fourier_mag = std::abs(fourier_table[constituent].first);
...
TEST_F(InferenceTest, AmplitudePhaseRoundtrip) {
  ...
  tide_table_[Constituent::kO1].first = Complex(expected_amp, expected_phase);
  tide_table_[Constituent::kK1].first = Complex(0.8, 45.0);
  tide_table_[Constituent::kQ1].first = Complex(0.2, 15.0);
  tide_table_[Constituent::kM2].first = Complex(1.0, 90.0);
  tide_table_[Constituent::kN2].first = Complex(0.3, 60.0);
  tide_table_[Constituent::kS2].first = Complex(0.6, 120.0);
  ...
  double result_amp = tide_table_[Constituent::kO1].first.real();
  double result_phase = tide_table_[Constituent::kO1].first.imag();
  ...
  EXPECT_GT(tide_table_[Constituent::kK1].first.real(), 0.0);
  EXPECT_GT(tide_table_[Constituent::kM2].first.real(), 0.0);
}
...
TEST_F(InferenceTest, ValidInputTypes) {
  ...
  EXPECT_NO_THROW({
    ...
    Inference inference2(InterpolationType::kFourierAdmittance,
                         ...
                         CalculationType::kMinorLongPeriod, constituent_table_);
  });
}
...
TEST_F(InferenceTest, ComplexNumberStructure) {
  ...
  tide_table_[Constituent::kO1].first = Complex(0.5, 0.3);
  tide_table_[Constituent::kK1].first = Complex(0.8, 0.1);
  tide_table_[Constituent::kQ1].first = Complex(0.2, 0.4);
  tide_table_[Constituent::kM2].first = Complex(1.0, 0.2);
  tide_table_[Constituent::kN2].first = Complex(0.3, 0.5);
  tide_table_[Constituent::kS2].first = Complex(0.6, 0.1);
  ...
  for (const auto& constituent : {Constituent::kRho1, Constituent::kTau1,
                                  Constituent::kNu2, Constituent::kMu2}) {
    Complex& value = tide_table_[constituent].first;
...
TEST_F(InferenceTest, DifferentInputScales) {
  ...
  small_table[Constituent::kO1].first = Complex(1e-6, 1e-7);
  small_table[Constituent::kK1].first = Complex(2e-6, 1e-7);
  small_table[Constituent::kQ1].first = Complex(0.5e-6, 1e-7);
  small_table[Constituent::kM2].first = Complex(3e-6, 2e-7);
  small_table[Constituent::kN2].first = Complex(1e-6, 0.5e-7);
  small_table[Constituent::kS2].first = Complex(2e-6, 1.5e-7);

  EXPECT_NO_THROW(inference(small_table));

  // Test with large values
  TideTable large_table = make_tide_table();
  large_table[Constituent::kO1].first = Complex(1000.0, 500.0);
  large_table[Constituent::kK1].first = Complex(2000.0, 800.0);
  large_table[Constituent::kQ1].first = Complex(500.0, 200.0);
  large_table[Constituent::kM2].first = Complex(3000.0, 1000.0);
  large_table[Constituent::kN2].first = Complex(1000.0, 400.0);
  large_table[Constituent::kS2].first = Complex(2000.0, 600.0);
...
TEST_F(InferenceTest, FrequencyRangeConsistency) {
  ...
  double o1_mag = std::abs(test_table[Constituent::kO1].first);
  double k1_mag = std::abs(test_table[Constituent::kK1].first);
  double q1_mag = std::abs(test_table[Constituent::kQ1].first);
  ...
  double m2_mag = std::abs(test_table[Constituent::kM2].first);
  double n2_mag = std::abs(test_table[Constituent::kN2].first);
  double s2_mag = std::abs(test_table[Constituent::kS2].first);
...
TEST(InferenceOceanTide, RealCase) {
  ...
  auto inference =
      Inference(InterpolationType::kLinearAdmittance, InputType::kHormonic,
                CalculationType::kMinor, constituent_table);

  tide_table[kQ1] =
      std::make_pair(Complex(2.044581413269043, -2.3776917457580566), false);
  tide_table[kO1] =
      std::make_pair(Complex(7.7220735549926758, 1.2257133722305298), false);
  tide_table[kK1] =
      std::make_pair(Complex(-7.9487228393554688, 5.2526679039001465), false);
  tide_table[kN2] =
      std::make_pair(Complex(-18.638496398925781, 3.0053455829620361), false);
  tide_table[kM2] =
      std::make_pair(Complex(-90.521110534667969, -20.603012084960938), false);
  tide_table[kS2] =
      std::make_pair(Complex(-24.581066131591797, -25.664165496826172), false);

  inference(tide_table);

  for(const auto& constituent : tide_table.keys()) {
    auto & tide = tide_table[constituent].first;
