...
class InferenceTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    tide_table_[Constituent::kQ1] = Complex(0.1, 0.05);     // Diurnal
    tide_table_[Constituent::kO1] = Complex(0.5, 0.2);      // Diurnal
    tide_table_[Constituent::kK1] = Complex(0.8, 0.3);      // Diurnal
    tide_table_[Constituent::kN2] = Complex(0.3, 0.1);      // Semidiurnal
    tide_table_[Constituent::kM2] = Complex(1.0, 0.4);      // Semidiurnal
    tide_table_[Constituent::kS2] = Complex(0.6, 0.2);      // Semidiurnal
    tide_table_[Constituent::kNode] = Complex(0.02, 0.01);  // Long period
    tide_table_[Constituent::kSa] = Complex(0.05, 0.02);    // Long period
    tide_table_[Constituent::kMm] = Complex(0.08, 0.03);    // Long period
  }
...
TEST_F(InferenceTest, OperatorHarmonicMinor) {
  ...
  EXPECT_NE(tide_table_[Constituent::kRho1], Complex(0.0, 0.0));
  EXPECT_NE(tide_table_[Constituent::kTau1], Complex(0.0, 0.0));
  EXPECT_NE(tide_table_[Constituent::kNu2], Complex(0.0, 0.0));
  EXPECT_NE(tide_table_[Constituent::kMu2], Complex(0.0, 0.0));

  // Check that magnitudes are reasonable (all constituents should have some
  // magnitude)
  EXPECT_GT(std::abs(tide_table_[Constituent::kQ1]), 0.0);
  EXPECT_GT(std::abs(tide_table_[Constituent::kO1]), 0.0);
  EXPECT_GT(std::abs(tide_table_[Constituent::kK1]), 0.0);
}
...
TEST_F(InferenceTest, OperatorAmplitudeMinor) {
  ...
  tide_table_[Constituent::kQ1] = Complex(0.1, 30.0);   // amp=0.1, phase=30°
  tide_table_[Constituent::kO1] = Complex(0.5, 45.0);   // amp=0.5, phase=45°
  tide_table_[Constituent::kK1] = Complex(0.8, 60.0);   // amp=0.8, phase=60°
  tide_table_[Constituent::kN2] = Complex(0.3, 15.0);   // amp=0.3, phase=15°
  tide_table_[Constituent::kM2] = Complex(1.0, 90.0);   // amp=1.0, phase=90°
  tide_table_[Constituent::kS2] = Complex(0.6, 120.0);  // amp=0.6, phase=120°
  ...
  EXPECT_GT(std::abs(tide_table_[Constituent::kRho1]), 0.0);
  EXPECT_GT(std::abs(tide_table_[Constituent::kTau1]), 0.0);
}
...
TEST_F(InferenceTest, OperatorLongPeriod) {
  ...
  EXPECT_NE(tide_table_[Constituent::kSsa], Complex(0.0, 0.0));
  EXPECT_NE(tide_table_[Constituent::kMf], Complex(0.0, 0.0));
  EXPECT_NE(tide_table_[Constituent::kMSf], Complex(0.0, 0.0));
}
...
TEST_F(InferenceTest, ReasonableMagnitudes) {
  ...
  double m2_magnitude = std::abs(tide_table_[Constituent::kM2]);
  double nu2_magnitude = std::abs(tide_table_[Constituent::kNu2]);
  double mu2_magnitude = std::abs(tide_table_[Constituent::kMu2]);
  ...
  double o1_magnitude = std::abs(tide_table_[Constituent::kO1]);
  double rho1_magnitude = std::abs(tide_table_[Constituent::kRho1]);
  double tau1_magnitude = std::abs(tide_table_[Constituent::kTau1]);
...
TEST_F(InferenceTest, ZeroInputValues) {
  ...
  for (const auto& constituent : {Constituent::kRho1, Constituent::kTau1,
    ...
    EXPECT_DOUBLE_EQ(zero_table[constituent].real(), 0.0);
    EXPECT_DOUBLE_EQ(zero_table[constituent].imag(), 0.0);
  }
}
...
TEST_F(InferenceTest, InterpolationConsistency) {
  ...
  for (const auto& constituent : {Constituent::kRho1, Constituent::kTau1}) {
    double linear_mag = std::abs(linear_table[constituent]);
    double fourier_mag = std::abs(fourier_table[constituent]);
...
TEST_F(InferenceTest, AmplitudePhaseRoundtrip) {
  ...
  tide_table_[Constituent::kO1] = Complex(expected_amp, expected_phase);
  tide_table_[Constituent::kK1] = Complex(0.8, 45.0);
  tide_table_[Constituent::kQ1] = Complex(0.2, 15.0);
  tide_table_[Constituent::kM2] = Complex(1.0, 90.0);
  tide_table_[Constituent::kN2] = Complex(0.3, 60.0);
  tide_table_[Constituent::kS2] = Complex(0.6, 120.0);
  ...
  double result_amp = tide_table_[Constituent::kO1].real();
  double result_phase = tide_table_[Constituent::kO1].imag();
  ...
  EXPECT_GT(tide_table_[Constituent::kK1].real(), 0.0);
  EXPECT_GT(tide_table_[Constituent::kM2].real(), 0.0);
}
...
TEST_F(InferenceTest, ComplexNumberStructure) {
  ...
  tide_table_[Constituent::kO1] = Complex(0.5, 0.3);
  tide_table_[Constituent::kK1] = Complex(0.8, 0.1);
  tide_table_[Constituent::kQ1] = Complex(0.2, 0.4);
  tide_table_[Constituent::kM2] = Complex(1.0, 0.2);
  tide_table_[Constituent::kN2] = Complex(0.3, 0.5);
  tide_table_[Constituent::kS2] = Complex(0.6, 0.1);
  ...
  for (const auto& constituent : {Constituent::kRho1, Constituent::kTau1,
                                  Constituent::kNu2, Constituent::kMu2}) {
    Complex value = tide_table_[constituent];
...
TEST_F(InferenceTest, DifferentInputScales) {
  ...
  small_table[Constituent::kO1] = Complex(1e-6, 1e-7);
  small_table[Constituent::kK1] = Complex(2e-6, 1e-7);
  small_table[Constituent::kQ1] = Complex(0.5e-6, 1e-7);
  small_table[Constituent::kM2] = Complex(3e-6, 2e-7);
  small_table[Constituent::kN2] = Complex(1e-6, 0.5e-7);
  small_table[Constituent::kS2] = Complex(2e-6, 1.5e-7);

  EXPECT_NO_THROW(inference(small_table));

  // Test with large values
  TideTable large_table = make_tide_table();
  large_table[Constituent::kO1] = Complex(1000.0, 500.0);
  large_table[Constituent::kK1] = Complex(2000.0, 800.0);
  large_table[Constituent::kQ1] = Complex(500.0, 200.0);
  large_table[Constituent::kM2] = Complex(3000.0, 1000.0);
  large_table[Constituent::kN2] = Complex(1000.0, 400.0);
  large_table[Constituent::kS2] = Complex(2000.0, 600.0);
...
TEST_F(InferenceTest, FrequencyRangeConsistency) {
  ...
  double o1_mag = std::abs(test_table[Constituent::kO1]);
  double k1_mag = std::abs(test_table[Constituent::kK1]);
  double q1_mag = std::abs(test_table[Constituent::kQ1]);
  ...
  double m2_mag = std::abs(test_table[Constituent::kM2]);
  double n2_mag = std::abs(test_table[Constituent::kN2]);
  double s2_mag = std::abs(test_table[Constituent::kS2]);
